...
export class DesignationsRepository extends BaseRepository {
  ...
  async create(tc: TenantContext, data: Partial<Designations>, ex?: Pool | PoolClient) {
    const r = await (ex || this.pool).query(`INSERT INTO designations (organization_id, name, code, parent_business_unit_id, status, created_by, updated_by) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`, [tc.organizationId, data.name, data.code, data.parent_business_unit_id || null, data.status || 'active', tc.userId, tc.userId]);
    return r.rows[0];
  }
  ...
  async findAll(tc: TenantContext, ex?: Pool | PoolClient) {
    const r = await (ex || this.pool).query(`SELECT * FROM designations WHERE organization_id = $1 ORDER BY name`, [tc.organizationId]);
    return r.rows;
  }
