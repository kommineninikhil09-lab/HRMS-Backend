...
export class DesignationsService {
  constructor(private repo: DesignationsRepository, private auditService: AuditService) {}
  async create(tc: TenantContext, data: any) {
    const d = await this.repo.create(tc, data);
    await this.auditService.record(tc, { action: 'CREATE', entity_type: 'designation', entity_id: d.id, new_value: d });
    return d;
  }
  ...
  async update(tc: TenantContext, id: string, data: any) {
    ...
    if (u) await this.auditService.record(tc, { action: 'UPDATE', entity_type: 'designation', entity_id: id, new_value: u });
    return u;
  }
  async delete(tc: TenantContext, id: string) {
    await this.repo.delete(tc, id);
    await this.auditService.record(tc, { action: 'DELETE', entity_type: 'designation', entity_id: id });
  }
}
